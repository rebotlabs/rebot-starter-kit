...
describe('AcceptInvitationJob', function () {
    ...
    it('accepts invitation for existing user', function () {
        ...
        Notification::assertSentTo($existingUser, EmailVerificationOtpNotification::class);
    });
    ...
    it('sets current organization for user without one', function () {
        ...
        $job = new AcceptInvitationJob($invitation->accept_token);
        $job->handle();
    ...
    it('does not create duplicate membership', function () {
        ...
        $this->organization->members()->create(['user_id' => $existingUser->id]);
        ...
        expect($this->organization->members()->where('user_id', $existingUser->id)->count())->toBe(1);
    });
    ...
    it('throws exception for already accepted invitation', function () {
        ...
        expect(fn () => (new AcceptInvitationJob($invitation->accept_token))->handle())
            ->toThrow(ModelNotFoundException::class);
    });
